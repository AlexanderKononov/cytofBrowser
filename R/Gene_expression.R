...
get_mk_clust_heatmap_dataframe <- function(fcs_raw, use_markers, cell_clustering, summarize_mode = 'median'){
  ...
  if(summarize_mode == 'median'){cluster_expr_median <- dplyr::summarize_all(cluster_expr_median, funs(median))}
  if(summarize_mode == 'mean'){cluster_expr_median <- dplyr::summarize_all(cluster_expr_median, funs(mean))}
...
get_deconvol_dataframe <- function(fcs_raw, use_markers, cell_clustering, summarize_mode = 'median'){
  ...
  if(summarize_mode == 'median'){summarised_cluster <- dplyr::summarise_all(summarised_cluster, median)}
